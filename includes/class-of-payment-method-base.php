...
require_once(plugin_dir_path(__FILE__) . 'class-auth-api.php');
require_once(plugin_dir_path(__FILE__) . 'class-merchant-api.php');
...
function find_item_case_insensitive($arr, $targetKey) {
...
function compare_header_case_insensitive($arr, $targetKey, $targetValue) {
  return strcasecmp(find_item_case_insensitive($arr, $targetKey), $targetValue) == 0;
}
...
class OF_Payment_Method_Base extends WC_Payment_Gateway {
  ...
  public function __construct($plugin_file) {
    ...
    $this->admin_scripts[$this->id . '_admin'] = 'admin.js';
    $this->admin_styles[$this->id . '_admin'] = 'admin.css';
    ...
    $this->decorators = array(
      new OF_Instrumenation_Support($plugin_data['Version']),
    );

    $this->wp_integration = new OF_Wordpress_Integration(
      $this->id, $this->plugin_file
    );
  ...
  function merchant_webhook_webhook() {
    ...
    if (!compare_header_case_insensitive($all_headers, $stored_webhook_auth_header, $stored_webhook_auth_value)) {
      error_log(print_r("Invalid webhook", TRUE));
      return;
    }

    $txn_ref_id = find_item_case_insensitive($payload["data"], "txn_ref_id");
    $txn_state = find_item_case_insensitive($payload["data"], "txn_state");
    $txn_pg_charge_id = find_item_case_insensitive($payload["data"], "txn_pg_charge_id");
  ...
  public function is_available() {
    ...
    return !empty($client_credentials)
        ...
        && get_transient("{$this->id}_tenant_id");
  }
